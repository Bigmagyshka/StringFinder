...
MyParserRunnable::MyParserRunnable(
	const QString &sPath, const QString &sIncludeText,
	const QString &sExcludeText, int nVersion, bool bUseOldStyleStream,
	bool bIsCaseSensetiveInclude, bool bSearchFullPhraseInclude,
	bool bIsCaseSensetiveExclude, bool bSearchFullPhraseExclude,
	QSharedPointer<bool> &bForceStop)
	: QObject(), QRunnable(), m_sPath(sPath),
	  ...
	  m_nVersion(nVersion), m_bUseOldStyleStream(bUseOldStyleStream),
...
bool MyParserRunnable::CheckIsTextSuitable(const QString &sText) {
  ...
  if (m_bSearchFullPhraseInclude) {
	...
	for (auto &str : m_vecIncludeText) {
	  if (!sText.contains(str, m_bIsCaseSensetiveInclude
  ...
  if (m_bSearchFullPhraseExclude) {
	...
	for (auto &str : m_vecExcludeText) {
	  if (!sText.contains(str, m_bIsCaseSensetiveExclude
								   ? Qt::CaseSensitivity::CaseSensitive
								   : Qt::CaseSensitivity::CaseInsensitive))
		continue;
...
void MyParserRunnable::UseQByteArray() {
  ...
  for (const auto &sLine : listSplited) {
	...
	pResult->addChild(new QTreeWidgetItem(
		QStringList{"", QString::number(nLine), sLineText}));
  }
  file.close();
