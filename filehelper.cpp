...
void WriteStringToOStream(QTextStream &osStream,
						  QPair<QString, QString> sKeyAndString) {
  osStream << sKeyAndString.first << "=" << sKeyAndString.second << Qt::endl;
}

bool WriteVecToFile(QString sFileName,
					QVector<QPair<QString, QString>> vecKeyAndString,
					bool bReWrite /*= true*/) {
  QFile file(sFileName);
  if (!file.open(QIODevice::WriteOnly |
				 (bReWrite ? QIODevice::NotOpen : QIODevice::Append)))
	return false;
